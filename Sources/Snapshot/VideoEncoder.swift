    ...
    init?(width: Int32, height: Int32, onEncodedFrame: @escaping (TimedSample) -> Void) {
        ...
        func createSession(codecType: CMVideoCodecType) -> VTCompressionSession? {
            ...
            guard status == noErr else {
                NSLog("%{public}@", "Snapshot: VTCompressionSessionCreate failed for codec \(codecType) (\(status))")
                return nil
            }
            return newSession
        }
        ...
        guard let session else {
            NSLog("%{public}@", "Snapshot: failed to create a VTCompressionSession with any supported codec")
            return nil
        }
